...
def main():
    ...
    while True:
        ...
        for chunk in client.stream(messages):
            if chunk.content:
                safe_chunk = selected_gr.process_chunk(chunk.content)
                if safe_chunk:
                    print(safe_chunk, end="", flush=True)
                    full_response += safe_chunk

        final_chunk = selected_gr.finalize()
        if final_chunk:
...
# ---------
# Guardrail that will prevent leaks of PII (output guardrail) in streaming mode.
# Flow:
#    -> user query
...
# ---------
# 1. Run the application and test such queries:
